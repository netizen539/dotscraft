...
namespace VoxelMaster
{
    ...
    public class VoxelGeneration : MonoBehaviour
    {
        ...
        //Component[] cubePrefabComponents;
        //ComponentType[] cubePrefabComponentTypes;
        public Mesh cubeMesh;
        public Material cubeMaterial;
        RenderMesh cubeRenderMesh;
        EntityManager em;
        Entity entityPrefab;
        ...
        void Start()
        {
            ...
            entityPrefab = em.CreateEntity();
            em.AddSharedComponentData(entityPrefab, cubeRenderMesh);
            var translation = new Translation() {};
            em.AddComponentData(entityPrefab, translation); 
            em.AddComponentData(entityPrefab, new Rotation()
            {
                Value = Quaternion.identity
            });
            em.AddComponentData(entityPrefab, new LocalToWorld());
        }
        ...
        public void GenerateEntitiesCoroutine(Chunk c)
        {
            for (int y = 0; y < c.size; y++)
            {
                for (int x = 0; x < c.size; x++)
                {
                    for (int z = 0; z < c.size; z++)
                    {
                        ...
                        Entity entity = em.Instantiate(entityPrefab);
                        float3 p = new float3(c.x + x, c.y+ y, c.z+ z);
                        var translation = new Translation() { Value = p };
                        em.SetComponentData(entity, translation); 
                        em.AddComponent(entity, typeof(BlockTagComponent));
                       // yield return null;
                    }
                }
            }
        ...
        void GenerateChunk(Vector3 v, int ChunkSize)
        {
            ...
            {
                c.SetBlocks(blocks);
                GenerateEntitiesCoroutine(c);

                //     StartCoroutine(GenerateEntitiesCoroutine(c));
            }
        }
        ...
        private Vector3[] GetChunkGridSimple(Vector3 pos, int chunkSize)
        {
            List<Pair> grid = new List<Pair>();
            Vector3 v = new Vector3(0.0f, 0.0f, 0.0f);
            if (!terrain.ChunkExists(v))
            {
